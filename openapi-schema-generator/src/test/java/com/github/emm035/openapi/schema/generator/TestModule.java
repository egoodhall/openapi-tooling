...
public class TestModule implements Module {
  ...
  public void configure(Binder binder) {
    OptionalBinder.newOptionalBinder(binder, Key.get(SchemaExtension.class, Extension.class))
      .setBinding()
      .toProvider(binder.getProvider(SchemaExtension.class));

    OptionalBinder.newOptionalBinder(binder, Key.get(PropertyExtension.class, Extension.class))
      .setBinding()
  ...
  PropertyExtension providesPropertyExtension() {
    return (schema, beanProperty) -> {
      System.out.println(beanProperty.getName() + " -> " + TypeUtils.toTypeName(beanProperty.getType()));
      return schema;
    };
  }
}
