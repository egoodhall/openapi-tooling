...
public class SchemaGeneratorExtensionTest {
  ...
  @Test
  public void schemaExtension_calledOnAllPrimitiveSchemas() throws SchemaGenerationException {
    ImmutableList.Builder<Schema> builder = ImmutableList.builder();

    SchemaGenerator generator = SchemaGenerator.builder()
      .bindSchemaExtension((schema, data) -> {
        builder.add(schema);
        return schema;
      }).build();
    ...
    Assertions.assertThat(builder.build()).containsExactlyInAnyOrder(
      IntegerSchema.builder()
        .setFormat(IntegerSchema.Format.INT32)
        .build(),
      IntegerSchema.builder()
        .setFormat(IntegerSchema.Format.INT64)
      .build(),
      NumberSchema.builder()
        .setFormat(NumberSchema.Format.DOUBLE)
      .build(),
      NumberSchema.builder()
        .setFormat(NumberSchema.Format.FLOAT)
      .build(),
  ...
  public void propertyExtension_calledOnAllProperties() throws SchemaGenerationException {
    ...
    SchemaGenerator generator = SchemaGenerator.builder()
      .bindPropertyExtension((schema, data) -> {
        builder.add(schema);
        return schema;
      }).build();
  ...
  public void schemaProperty_overridesPropertyName() throws SchemaGenerationException {
    ...
    Referenceable<Schema> referenceable = generator.generateSchema(new TypeReference<SimpleGeneric<String>>() {
    });
    ObjectSchema schema = (ObjectSchema) generator.resolve(referenceable);
    Assertions.assertThat(schema.getProperties().keySet()).containsExactlyInAnyOrder(
      "type",
      "overridden-property"
    );
  }
